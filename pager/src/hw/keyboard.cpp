...
//   row2 col0 = Alt   row2 col8 = Caps   row2 col9 = Backspace
//   row3 col0 = Sym   row3 col1..9 = space bar contacts
...
constexpr uint8_t kKeySym = 30;   // row3 col0
...
bool poll(Event *out) {
  ...
  // Orange key: hold + letter = number/symbol (BB style); tap latches the
  // layer for one key; tap again cancels. Alt is treated the same as Sym so
  // the orange key works whichever matrix slot this PCB revision uses.
  if (k == kKeySym || k == kKeyAlt) {
    symDown = true;
    symUsed = false;
    sym = !sym;
